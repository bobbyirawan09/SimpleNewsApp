package bobby.irawan.simplenewsapp.presentation.presentation.newscategory
...
import bobby.irawan.simplenewsapp.presentation.presentation.adapter.NewsAdapter
import bobby.irawan.simplenewsapp.presentation.presentation.newsdetail.NewsDetailActivity
...
class NewsCategoryFragment : BaseFragment<FragmentNewsBinding>(), NewsAdapter.ClickListener {
    ...
    override fun main(view: View, savedInstanceState: Bundle?) {
        ...
        setToolbar(args.category?.name)
        newsViewModel.getNewsDataWithCategory(args.category?.name.orEmpty())
    }

    private fun setToolbar(name: String?) {
        binding.toolbar.text_view_toolbar_title.text = name.orEmpty()
